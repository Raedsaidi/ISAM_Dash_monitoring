...
def create_user_admin(
    ...
    - On impose au moins 1 port dans user_in.ports.
    """
    ...
    if not user_in.ports or len(user_in.ports) == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Au moins un port est requis pour l'utilisateur.",
        )
    ...
    if not created_ports:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Tous les ports fournis sont vides. Au moins un port valide est requis.",
        )
...
def change_user_role(
    ...
    if user.role == UserRole.SUPER_ADMIN.value:
        raise HTTPException(
            status_code=403,
            detail="Impossible de modifier le rôle d'un SUPER_ADMIN.",
        )

    user.role = role_request.role.value
    db.add(user)
    db.commit()
    db.refresh(user)
    return user